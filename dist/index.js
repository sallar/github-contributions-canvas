...
function getContributionCount(graphEntries) {
  return graphEntries.reduce((rowTotal, row) => {
    return rowTotal + row.reduce((colTotal, col) => {
...
function drawYear(ctx, year, offsetX = 0, offsetY = 0, data) {
  const thisYear = (0, _moment.default)().format("YYYY");
  const today = year.year === thisYear ? (0, _moment.default)() : (0, _moment.default)(year.range.end);
  const start = (0, _moment.default)(`${year.year}-01-01`);
  ...
  for (let i = 1; i < 7; i += 1) {
    graphEntries.push(firstRowDates.map(dateObj => {
      const date = (0, _moment.default)(dateObj.date).day(i).format(FORMAT);
      return {
        date,
        info: getDateInfo(data, date)
      };
  ...
  ctx.fillText(`${year.year}: ${count} Contribution${year.total === 1 ? "" : "s"}${thisYear === year.year ? " (so far)" : ""}`, offsetX, offsetY);
...
function drawContributions(canvas, data, username) {
